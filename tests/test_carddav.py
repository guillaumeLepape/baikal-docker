...
def test_put_and_get_contact_round_trips(baikal_server):
    ...
    get_resp = requests.get(contact_url, auth=_auth(baikal_server))
...
def test_delete_contact(baikal_server):
    ...
    delete_resp = requests.delete(contact_url, auth=_auth(baikal_server))
...
def test_unauthenticated_request_is_rejected(baikal_server):
    resp = requests.get(_addressbook_url(baikal_server))
