...
def test_put_and_get_event_round_trips(baikal_server):
    ...
    get_resp = requests.get(event_url, auth=_auth(baikal_server))
...
def test_delete_event(baikal_server):
    ...
    delete_resp = requests.delete(event_url, auth=_auth(baikal_server))
...
def test_unauthenticated_request_is_rejected(baikal_server):
    resp = requests.get(_calendar_url(baikal_server))
